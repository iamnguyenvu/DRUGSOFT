...
public class DangNhap_DAO {
	public Integer getRole(String username, String password) {
        ...
        Integer role = null;
        try {
            ps = con.prepareStatement("SELECT phanQuyen FROM TaiKhoan WHERE tenDangNhap = ? AND matKhau = ? AND trangThai = 1");
	...
	public String getHoTen(String username) {
        ...
        String hoTen = null;
        try {
            ps = con.prepareStatement("SELECT hotenNV FROM NhanVien WHERE maNV = ? AND trangThai = 1");
            ps.setString(1, username);
            rs = ps.executeQuery();
            if (rs.next()) {
