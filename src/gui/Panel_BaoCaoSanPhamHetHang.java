...
import dao.BaoCaoSanPhamHetHang_DAO;
 ...
 * @author Vovan
 */
public class Panel_BaoCaoSanPhamHetHang extends javax.swing.JPanel {

        private int currentPage = 1;
        private int rowsPerPage = 10;
        private int totalPages = 0;
        private BaoCaoSanPhamHetHang_DAO dao;
        private JDialog dateDialog;
        private Date selectedStartDate;
        private Date selectedEndDate;
        private int selected;
        private List<BaoCaoSanPhamHetHang_enity> reportData;
     ...
     */
    public Panel_BaoCaoSanPhamHetHang() {
        initComponents();
        addActionListeners();
    ...
    public void loadData(int currentPage, int rowsPerPage) {
        ...
        dao = new BaoCaoSanPhamHetHang_DAO();
        ArrayList<BaoCaoSanPhamHetHang_enity> dsBaoCao = dao.getAllBaoCaoSanPhamHetHang();
        ...
        for (int i = start; i < end; i++) {
            BaoCaoSanPhamHetHang_enity bc = dsBaoCao.get(i);
            Object[] rowData = {
                bc.getMaSP(),
                bc.getTenSP(),
                bc.getLoaiSP(),
    ...
    try {
        ...
        Font footerFont = new Font(baseFont, 13, Font.BOLD);
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            // Thêm thông tin ngày lập
            LocalDate currentDate = LocalDate.now();
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            Paragraph reportDate = new Paragraph("Ngày lập báo cáo: " + currentDate.format(formatter), footerFont);
            ...
            // Lấy dữ liệu báo cáo
            List<BaoCaoSanPhamHetHang_enity> reportData = dao.getAllBaoCaoSanPhamHetHang();

            // Tạo bảng PDF
            PdfPTable pdfTable = new PdfPTable(5); // 5 cột: Mã SP, Tên SP, Loại SP, Giá, Số lượng tồn
            pdfTable.setWidthPercentage(100);
            pdfTable.setSpacingBefore(10f);
            pdfTable.setSpacingAfter(10f);

            // Thêm tiêu đề bảng
            String[] headers = {"Mã sản phẩm", "Tên sản phẩm", "Loại sản phẩm", "Giá (VNĐ)", "Số lượng tồn"};
            ...
            for (BaoCaoSanPhamHetHang_enity item : reportData) {
                pdfTable.addCell(new PdfPCell(new Phrase(item.getMaSP(), contentFont)));
                pdfTable.addCell(new PdfPCell(new Phrase(item.getTenSP(), contentFont)));
                pdfTable.addCell(new PdfPCell(new Phrase(item.getLoaiSP(), contentFont)));
                pdfTable.addCell(new PdfPCell(new Phrase(String.format("%,.2f", item.getGia()), contentFont)));
                pdfTable.addCell(new PdfPCell(new Phrase(String.valueOf(item.getSlt()), contentFont)));
            }
            ...
            Paragraph footer = new Paragraph("Người lập báo cáo", footerFont);
            footer.setAlignment(Element.ALIGN_RIGHT);
            footer.setIndentationRight(50f);
            document.add(footer);

            Paragraph signature = new Paragraph("(Ký và ghi rõ họ tên)", footerFont);
            signature.setAlignment(Element.ALIGN_RIGHT);
            signature.setIndentationRight(50f);
    ...
    try {
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            Sheet sheet = workbook.createSheet("Báo cáo sản phẩm sắp hết hàng");
            ...
            titleCell.setCellValue("BÁO CÁO SẢN PHẨM SẮP HẾT HÀNG");

            // Căn giữa tiêu đề báo cáo
            ...
            for (BaoCaoSanPhamHetHang_enity item : reportData) {
                ...
                row.createCell(3).setCellValue(item.getGia());
                row.createCell(4).setCellValue(item.getSlt());

                // Áp dụng kiểu cho tất cả các ô trong hàng
                for (int i = 0; i < 5; i++) {
