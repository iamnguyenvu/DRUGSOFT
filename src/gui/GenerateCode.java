...
//        System.out.println("Barcode created for " + maSP + " at: " + path.toString());
    ...
    private static String decodeQrCode(BufferedImage bufferedImage) throws IOException, NotFoundException, ChecksumException, FormatException {
        ...
        // Chuyển đổi thành BinaryBitmap
        BinaryBitmap bitmap = new BinaryBitmap(new HybridBinarizer(source));

        // Sử dụng QRCodeReader để giải mã QR Code
    ...
    public static String startQrcodeScanner() {
        ...
        AtomicReference<String> scannedCode = new AtomicReference<>(null);
        ...
        javax.swing.Timer timer = new javax.swing.Timer(30, e -> {
            if (camera.read(frame)) {
                try {
                    BufferedImage bufferedImage = matToBufferedImageQrcode(frame);
        ...
        while (scannedCode == null) {
            try {
                Thread.sleep(100); // Add a small delay to prevent busy-wait
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
        
        return scannedCode.get();
    }
