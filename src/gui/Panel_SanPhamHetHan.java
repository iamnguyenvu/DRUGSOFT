 ...
 * @author RAVEN
 */
public class Panel_SanPhamHetHan extends SimpleForm implements ActionListener{
     ...
     */
    public Panel_SanPhamHetHan() {
    	tk_Dao = new ThongKe_DAO();
		setPreferredSize(new Dimension(1117, 650));
        initComponents();
        ...
        String[] columnNames = { "Mã Sản Phẩm", "Tên Sản Phẩm","Loại SP", "Giá","Đơn Vị Tính","Thuế", "Số Lượng"
				};
    ...
    public void docDataVaoTableSapHetHan() {
        ...
        for (SanPham_entity product : products) {
            dftb_SanPham.addRow(new Object[] {
                product.getMaSP(), product.getTenSP(),product.getLoaiSanPham().getMaLoaiSP(),product.getGia(),product.getDonViTinh(),product.getThue(),product.getSoLuong()
            });
        }
    }
