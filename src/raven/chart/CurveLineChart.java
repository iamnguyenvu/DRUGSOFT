...
//				if (!model.isEmpty() && animate > 0 && index >= 0 && index < legends.size()) {
//					// Đặt điều kiện để giới hạn số lượng nhãn trên trục x
//					int maxLabels = 10; // Giới hạn số lượng nhãn tối đa trên trục x
//					int step = Math.max(1, model.size() / maxLabels); // Bỏ qua một số nhãn dựa trên số lượng dữ liệu
//
//					for (int i = 0; i < model.size(); i += step) {
//						// Đảm bảo rằng chỉ số 'i' không vượt quá kích thước của legends
//						if (i < legends.size()) {
//							draw(g2, rectangle, i,
...
//			}
			...
			public void renderGraphics(BlankPlotChart chart, Graphics2D g2, Rectangle2D rectangle) {
				if (!model.isEmpty() && animate > 0 && index >= 0 && index < legends.size()) {
					...
					int maxLabels = (int) (rectangle.getWidth() / (fm.getHeight() + 10)); // Adjust label spacing
					int step = Math.max(1, model.size() / maxLabels);

					for (int i = 0; i < model.size(); i += step) {
						// Ensure we do not exceed legends size
						if (i < legends.size()) {
							draw(g2, rectangle, i,
									chart.getNiceScale().getTickSpacing() * chart.getNiceScale().getMaxTicks());
						}
	...
	private SplinePoint[] toPoint(Rectangle2D rec, int index, double maxValue) {
		...
		// Kiểm tra xem model có kích thước hợp lệ không
		if (model.size() > 0) {
			for (int i = 0; i < model.size(); i++) {
				double[] values = model.get(i).getValues();
				// Kiểm tra xem index có hợp lệ với kích thước của values không
				if (index < values.length) {
					points[i + 1] = toPoint(rec, i, model.size(), values[index], maxValue);
				} else {
					// Xử lý trường hợp index không hợp lệ
					points[i + 1] = toPoint(rec, i, model.size(), 0, maxValue); // Hoặc giá trị mặc định
				}
			}
			points[0] = points[1]; // Đảm bảo có phần tử trước khi gán
			points[points.length - 1] = points[points.length - 2]; // Đảm bảo có phần tử trước khi gán
		}
